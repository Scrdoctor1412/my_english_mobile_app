...
class HiveDatabase implements LocalData {
  ...
  Future<void> saveScheduleNotification(
    ...
    return _appHive.scheduleNotificationBox.put(
  ...
  Future<void> saveWords(List<Word> words) async {
    await _appHive.wordBox.putAll(
      Map.fromEntries(words.map((word) => MapEntry(word.id, word))),
    );
  }
  ...
  Future<void> saveLearningCategories(
    List<LearningCategory> learningCategory,
  ) async {
    await _appHive.learningCategoryBox.putAll(
      Map.fromEntries(learningCategory.map((e) => MapEntry(e.id, e))),
    );
  }

  @override
  Future<void> updateLearningCategory(LearningCategory learningCategory) async {
    await _appHive.learningCategoryBox.put(
      learningCategory.id,
      learningCategory,
    );
  }
  ...
  Future<void> saveLeitnerBoxes(List<LeitnerBox> leitnerBoxes) async {
    await _appHive.leitnerBoxBox.putAll(
      Map.fromEntries(leitnerBoxes.map((e) => MapEntry(e.index, e))),
    );
  }
  ...
  Future<void> saveRecords(List<LearningRecord> learningRecords) async {
    await _appHive.learningRecordBox.putAll(
      Map.fromEntries(learningRecords.map((e) => MapEntry(e.id, e))),
    );
  }
}
